...
Données récentes sur la pollition NOX à Toulouse
```{python}
...
viz.visualize_df(df=df_j, city='TOULOUSE', nom_poll="NOX")

```

Données mensuelles sur la pollition NOX à Toulouse
```{python}
# Show NOX Pollution Values of the last months
df_m=dp.data_prep(time='m')
viz.visualize_df(df=df_m, city='TOULOUSE', nom_poll="NOX")
