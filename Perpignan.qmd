...
Données récentes sur la pollition NOX à Perpignan
```{python}
...
viz.visualize_df(df=df_j, city='PERPIGNAN', nom_poll="NOX")

```

Données mensuelles sur la pollition NOX à Perpignan
```{python}
# Show NOX Pollution Values of the last months
df_m=dp.data_prep(time='m')
viz.visualize_df(df=df_m, city='PERPIGNAN', nom_poll="NOX")
